...
class ZoneCities extends CActiveRecord
{
	...
	public static function getZonesByCityIds($cityIds)
	{
		$qry = "SELECT GROUP_CONCAT(zct_zon_id) as zones FROM zone_cities 
						INNER JOIN zones ON zon_id=zct_zon_id AND zon_active=1
						WHERE zct_cty_id IN ({$cityIds}) AND zct_active=1";

		$zones = DBUtil::queryScalar($qry, DBUtil::SDB(), [], 60 * 60 * 24, CacheDependency::Type_Zones);
		return $zones;
	}
	...
	{
		$qry		 = "SELECT GROUP_CONCAT(DISTINCT zct_zon_id) as zones FROM zone_cities WHERE zct_cty_id IN($city_ids) AND zct_active=1";
		$recordset	 = DBUtil::command($qry)->queryScalar();
		return $recordset;
	}
