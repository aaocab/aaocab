...
class SmsLog extends CActiveRecord
{
	...
	public function statusUpdateById($slgId, $response, $provider = 0)
	{
		...
		$model->slg_provider_type	 = $provider;
		if ($model->save())
		{
			return true;
		}
	 ...
	 * @return boolean|int
	 */
	public static function getCountByType($phncode, $type, $phnNumber = '', $noOfHRvalid = 0, $sendType = 0, $dateTime = '')
	{
		$condition = ' AND 1=1';
		if ($type == '')
		{
		...
		if ($dateTime == "")
		{
			$condition .= " AND DATE(date_sent) = CURRENT_DATE";
		}
		...
		if ($noOfHRvalid > 0)
		{
			$condition .= " AND DATE_SUB(NOW(), INTERVAL $noOfHRvalid HOUR) <= date_sent";
		}

		$sql	 = "SELECT count(*) from sms_log WHERE slg_type=:slg_type AND status=1 $condition";
		$count	 = DBUtil::command($sql, DBUtil::SDB())->queryScalar(['slg_type' => $type]);
