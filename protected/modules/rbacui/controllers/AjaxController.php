...
class AjaxController extends RbController
{
	...
	public function actionGetUsers()
	{
		...
		if (isset($_GET['term']))
		{
			...
			$criteria->compare("adm_fname", $_GET['term'], true, ' OR ');
			$criteria->compare("adm_lname", $_GET['term'], true, ' OR ');
			$criteria->compare("adm_user", $_GET['term'], true, ' OR ');
			$criteria->addCondition("adm_active=1");
			$criteria->limit = 15;
			...
			{
				$json[] = array('value' => $model->getAttribute($this->module->userIdColumn), 'label' => $model->getName());
			}
		}
		echo json_encode($json);
		Yii::app()->end();
	...
	private function getAssignedUsers($item)
	{
		$auth = Yii::app()->authManager;
		$criteria = new CDbCriteria;
		$criteria->order = "adm_fname asc, adm_lname asc, adm_user ASC";
		if ($this->module->userActiveScope)
		{
			$criteria->scopes = $this->module->userActiveScope;
		}
