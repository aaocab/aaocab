...
$bookingType		 = ($followUps->queueType > 0 && $followUps->queueType == '53') ? "block" : "none";
?>
<div class="row">
    <div class="col-xs-12  pb10">
        <a href="/report/scq/scqreport" target="_blank"> Click To View  SCQ Report</a>
        <br>
        <a href="/report/scq/cbrStaticalDetailsData?fromdate=<?php echo $followUps->from_date; ?>&todate=<?php echo $followUps->to_date; ?>" target="_blank" > Click To View CBR Statistical Data Report</a>
    </div>
</div>
<div class="row">
	<?php
    ...
    <div class='row p15'>
        ...
        <div class='row p15'>
            ...
            <div class="col-xs-12 col-sm-3 col-md-2"> 
                <div class="form-group">
					...
					<?php
					$arrData				 = QueueMaster::model()->getList();
					...
					$queueTypeJson			 = Filter::getJSON($arrData);
		...
		<div class="row p15">
			...
			<div class="col-xs-12 col-sm-2 col-md-2" id="bookingType" style="display: <?= $bookingType ?>"> 
                <div class="form-group">
                    <label class="control-label" style="margin-left:5px;">Booking Type</label>
					<?php
					$bookingTypeJson	 = Filter::getJSON(array("0" => "All", "1" => "Quote Booking", "2" => "Confirm Booking"));
					$this->widget('booster.widgets.TbSelect2', array(
						'model'			 => $followUps,
						'attribute'		 => 'bookingType',
						'val'			 => $followUps->bookingType,
						'asDropDownList' => FALSE,
						'options'		 => array('data' => new CJavaScriptExpression($bookingTypeJson), 'allowClear' => true),
        ...
        <div class='row p15'>
			...
			if ($checkExportAccess)
			{
				...
				echo CHtml::beginForm(Yii::app()->createUrl('report/scq/cbrdetailsreport'), "post", ['style' => "margin-bottom: 10px;margin-top: 10px;"]);
				?>
				<div class="col-xs-12 col-sm-2 col-md-2">   
					<label class="control-label"></label>
					...
					<input type="hidden" id="export_bookingType" name="export_bookingType" value="<?= $followUps->bookingType ?>"/>
...
<script type="text/javascript">
    $(document).ready(function () {
        $(document).on('click', '.submitCbr', function () {
            ...
            var bookingType = $("#ServiceCallQueue_bookingType").val();
            ...
            window.location.href = '/report/scq/cbrdetailsreport/?queueType=' + queueType + '&event_id=' + event_id + '&event_by=' + event_by + "&csrId=" + csrSearch + "&teamId=" + teamId + "&isCreated=" + isCreated + "&fromdate=" + fromdate + "&todate=" + todate + "&followupPerson=" + followupPerson + "&followupPersonEntity=" + followupPersonEntity + "&followupWith=" + followupWith + "&followupWithEntityType=" + followupWithEntityType + '&bookingType=' + bookingType;
        });
    ...
    $('#ServiceCallQueue_queueType').change(function () {
        var queueVal = $('#ServiceCallQueue_queueType').val();
        if (queueVal === '53') {
            $("#bookingType").show("slow");
            $("#ServiceCallQueue_bookingType").select2('data', {id: "0", text: "ALL"});
        } else
        {
            $("#bookingType").hide("slow");
        }
    });
