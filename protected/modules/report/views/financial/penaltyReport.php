            ...
            <div class="col-xs-12">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-12 col-sm-2 col-md-2 form-group ">
                        <?php
						$penaltyStatus	 = [1 => 'Applied', 2 => 'Removed'];
						$statusJSON		 = VehicleTypes::model()->getJSON($penaltyStatus);
						?>
                                <label class="control-label">Penalty Status</label>
								<?php
								$this->widget('booster.widgets.TbSelect2', array(
									'model'			 => $model,
									'attribute'		 => 'penalty_status',
									'val'			 => $model->penalty_status,
                ...
                <div class="row">
                    <div class="col-xs-12 text-center">
                        <div class="col-xs-offset-3 col-sm-offset-0 col-xs-6 col-sm-2 col-md-2 text-center" style="padding: 4px; margin-left: 40%;">
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="col-xs-12  ">
								...
								<input type="hidden" id="export_penalty_status" name="export_penalty_status" value="<?= $model->penalty_status; ?>">
                ...
                <div class="  table table-bordered">
					...
					if (!empty($dataProvider))
					{
						...
						$this->widget('booster.widgets.TbExtendedGridView', array(
							...
							'rowCssClassExpression'	 => '($data["act_active"]==1 && $data["ledgerAmt"] > 0) ? "bg bg-light-green" : "bg bg-light-red"',
							...
							'columns'				 => array(
								...
								array('name'	 => 'adt_trans_ref_id',
									'value'	 => function ($data) {
										if($data['adt_type'] ==1)
										{
											echo CHtml::link($data['adt_trans_ref_id'], Yii::app()->createUrl("admin/booking/view", ["id" => $data['adt_trans_ref_id']]), ["class" => "",  'target' => '_blank']);
									
										}
										else
										{
											echo $data['adt_trans_ref_id'] . "<br>(Trip)";
										}
										}, 'sortable'			 => false, 'headerHtmlOptions'	 => array('class' => 'col-xs-1 text-center'), 'htmlOptions'		 => array('class' => 'text-right'), 'header'			 => 'BKG ID'),
								array('name'	 => 'bkg_booking_id',
									'value'	 => function ($data) {
										if($data['adt_type'] ==1)
										{
										echo Booking::model()->findByPk($data['adt_trans_ref_id'])->bkg_booking_id;
										}
										else{
											echo '';
										}
								...
								array('name'	 => 'bkg_agent_ref_code',
									'value'	 => function ($data) {
											if($data['adt_type'] ==1)
										{
										echo Booking::model()->findByPk($data['adt_trans_ref_id'])->bkg_agent_ref_code;
										}
										else{
											echo '';
										}
									}
									, 'sortable'			 => false, 'headerHtmlOptions'	 => array('class' => 'col-xs-1 text-center'), 'htmlOptions'		 => array('class' => 'text-right'), 'header'			 => 'Agent Booking ID'),
								array('name'	 => 'vnd_code',
									'value'	 => function ($data) {
										echo $data['vnd_code'];
									}
								...
								array('name'	 => 'adt_remarks',
									'value'	 => function ($data) {
										if ($data['act_active'] == 1 && $data['ledgerAmt'] > 0)
										{
								...
								array('name'	 => 'act_user_id',
									'value'	 => function ($data) {
										if ($data['act_active'] == 1 && $data['ledgerAmt'] > 0)
										{
											...
											else if ($data['adm_id1'] != '')
											{
												echo $data['adm_fname1'] . '' . $data['adm_lname1'];
											}
											else
											{
												echo "System";
								...
								array('name'	 => 'adt_modified',
									'value'	 => function ($data) {
										if ($data['act_active'] == 0 || ($data['act_active'] == 1 && $data['ledgerAmt'] < 0))
										{
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var start = '<?= date('d/m/Y', strtotime($model->trans_create_date1)); ?>';
        var end = '<?= date('d/m/Y', strtotime($model->trans_create_date2)); ?>';
        $('#bkgCreateDate').daterangepicker(
                {
                    ...
                    ranges: {
                        'Today': [moment(), moment()],
                        'Yesterday': [moment().subtract(1, 'days'), moment().subtract(1, 'days')], 'Last 7 Days': [moment().subtract(6, 'days'), moment()],
