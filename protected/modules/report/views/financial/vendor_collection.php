			...
			?>
				<div class="panel-body">
					<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
						<?= CHtml::beginForm(Yii::app()->createUrl('report/financial/vendorcollection'), "post", []); ?>
						<div class="input-row">
							...
							<label><b>Select Day Range</b></label><br/>
							<select name="export_day_range" style="padding:5px;">
							...
							<br /><br />
							<button class="btn btn-primary" type="submit" style="width: 185px;">Export Table By Day Range</button>
						</div>
						<?= CHtml::endForm() ?>
					</div>
					<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
						...
						?>
						<div class="input-row">
							<label><b>Import/ Update Vendor Payout</b></label>
							<input type="file" name="file" id="file" accept=".csv" required="required">
							<div id="response"></div>
							<br />
							<button type="submit" id="submit" name="import" class="btn btn-primary" onclick="this.style.display='none';">Import Vendor Payout CSV File</button>
							<br />
							<?php
							if ($message != '')
							{
								echo '<div class="text-danger"><b>' . $message . '</b></div>';
							}
							?>
						</div>
						<?php
					...
					<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
						<label><b>Import/ Update Vendor Bank Beneficiary Id</b></label>
						<?php
						$form = $this->beginWidget('CActiveForm', array(
							'id'					 => 'frmBeneficiaryCSVImport',
							'enableAjaxValidation'	 => true,
							'htmlOptions'			 => array('enctype' => 'multipart/form-data'),
						));
						?>
						<div class="input-row">
							<input type="file" name="fileBeneficiary" id="fileBeneficiary" accept=".csv" required="required">
							<div id="responseBeneficiary"></div>
							<br />
							<button type="submit" id="updateBeneficiary" name="updateBeneficiary" class="btn btn-primary" onclick="this.style.display='none';">Import/ Update Vendor Bank Beneficiary Id CSV File</button>
							<br />
							<?php
							if ($messageBen != '')
							{
								echo '<div class="text-danger"><b>' . $messageBen . '</b></div>';
							}
							?>
						</div>
						<?php
						...
						$this->widget('booster.widgets.TbExtendedGridView', array(
							...
							'columns'			 => array(
								...
								array('name'	 => 'rating', 'type'	 => 'raw', 'value'	 => function ($data) {
									...
									'header'			 => 'Rating'),
								array('name' => 'last_login', 'value' => '$data[apt_last_login]', 'sortable' => false, 'headerHtmlOptions' => array('class' => 'col-xs-1 text-center'), 'htmlOptions' => array('class' => 'text-center'), 'header' => 'Last login date'),
								array('name' => 'home_zone', 'value' => '$data[cty_name]', 'sortable' => false, 'headerHtmlOptions' => array('class' => 'col-xs-1 text-center'), 'htmlOptions' => array('class' => 'text-center'), 'header' => 'Home zone'),
								array('name' => 'vrs_dependency', 'value' => '$data[vrs_dependency]', 'sortable' => false, 'headerHtmlOptions' => array('class' => 'col-xs-1 text-center'), 'htmlOptions' => array('class' => 'text-center'), 'header' => 'Dependency score'),	
								array(
									'header'			 => 'Action',
									'class'				 => 'CButtonColumn',
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		$("#frmBeneficiaryCSVImport").on("submit", function () {

			$("#responseBeneficiary").attr("class", "");
			$("#responseBeneficiary").html("");
			var fileType = ".csv";
			var regex = new RegExp("([a-zA-Z0-9\s_\\.\-:])+("
					+ fileType + ")$");
			if (!regex.test($("#fileBeneficiary").val().toLowerCase())) {
				$("#responseBeneficiary").addClass("error");
				$("#responseBeneficiary").addClass("display-block");
				$("#responseBeneficiary").html(
						"Invalid File. Upload : <b>" + fileType
						+ "</b> Files required.");
				return false;
