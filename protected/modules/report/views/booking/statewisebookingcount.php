...
<?php
	...
	{
	echo CHtml::beginForm(Yii::app()->createUrl('report/booking/StateWiseBookingCount'), "post", ['style' => "margin-bottom: 10px;margin-top: 10px;"]);
	?>
...
if (!empty($dataProvider))
{
	...
	$this->widget('booster.widgets.TbGridView', array(
		...
		array
			...
			array('name' => 'PickupMonth', 'value' => $data['PickupMonth'], 'sortable' => true, 'headerHtmlOptions' => array('class' => ''), 'header' => 'Pickup Month'),
			array('name' => 'StateName', 'value' => $data['StateName'], 'sortable' => true, 'headerHtmlOptions' => array('class' => ''), 'header' => 'State Name'),
		)
	));
}
?>
...
<script>
	...
	var createStart = '<?= ($model->bkg_pickup_date1 == '') ? date('d/m/Y', strtotime("-1 month", time())) : date('d/m/Y', strtotime($model->bkg_pickup_date1)); ?>';
	var createEnd = '<?= ($model->bkg_pickup_date2 == '') ? date('d/m/Y') : date('d/m/Y', strtotime($model->bkg_pickup_date2)); ?>';
