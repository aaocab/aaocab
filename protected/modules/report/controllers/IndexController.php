...
class IndexController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow', // allow all users to perform 'index' and 'view' actions
				'actions'	 => array('changepassword', 'dashboard', 'dashbordold', 'logout', 'saveDialer', 'redirectPerson'),
				'users'		 => array('@'),
			),
	...
	public function actionDashboard()
	{
		...
		$this->layout	 = 'admin1';
		$reportMenu		 = Report::getAllMenu();
		$outputJs		 = Yii::app()->request->isAjaxRequest;
		$method			 = "render" . ($outputJs ? "Partial" : "");
		$this->$method('dashboard_new', array("reportMenu" => $reportMenu), false, $outputJs);
	}
	...
	public function actionDownloadDocs()
	{
		...
		$id			 = Yii::app()->request->getParam('reportExportId');
		$model		 = ReportExport::model()->findByPk($id);
		if ($model->rpe_file_name == $filename && ($model->rpe_status == 1))
		{
