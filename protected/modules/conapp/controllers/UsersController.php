...
class UsersController extends BaseController
{
	...
	public function restEvents()
	{
		...
		$this->onRest('post.filter.req.auth.user', function ($validation) {
			...
			$ri	 = array('/signIn', '/signUp', '/sosTrigger', '/changePassword', '/registerDevice', '/forgotPass',
				...
				'/userLogout', '/updateSosTrigger', '/validateApp', '/sosAddContact', '/updateFcm', '/generateReferCode', '/updateProfile', '/sendOtp', '/verifyOTP', '/verifyUser');
	...
	public function sendOtp()
	{
		...
		try
		{
			...
			Contact::verifyOTP($objCttVerify, $canSendSMS);
			$contactDetails = Yii::app()->JWT->encode([["type" => $objCttVerify->type, "value" => $objCttVerify->value, "isSendSMS" => $objCttVerify->isSendSMS]]);
			//$contactObj = new Stub\common\ContactVerification($objCttVerify->type, $objCttVerify->value);
			//$rData =  $objPage->getEncrptedData();
...
//			}
	...
	public function updateProfileImage()
	{
		...
		try
		{
			...
			$returnSet->setStatus(true);
			$returnSet->setData(["userProfilePic" => $model->usr_profile_pic]);
			DBUtil::commitTransaction($transaction);
			Logger::create("Response : " . json_encode($returnSet), CLogger::LEVEL_INFO);
		}
		catch (Exception $e)
