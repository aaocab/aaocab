			...
			<div class="panel panel-default panel-border">
				<div class="panel-body">
					<div class="row mb15">
						...
						<div class="col-xs-12 col-md-6 mb15" id="groupByperiod">
							...
							$this->widget('booster.widgets.TbSelect2', array(
								'model'			 => $model,
								'attribute'		 => 'groupby_period',
								'val'			 => $model->groupby_period,
								'asDropDownList' => FALSE,
								'options'		 => array('data' => new CJavaScriptExpression($dataPay), 'allowClear' => true),
								'htmlOptions'	 => array('class' => 'p0', 'style' => 'width:100%', 'placeholder' => 'Select Group By')
							));
							?>
						...
						<div class="col-xs-12 col-md-6 mb15" id="groupByVender"  style="display:none">
							<?=
				...
				if (!empty($dataprovider))
				{
					$this->widget('booster.widgets.TbGridView', array(
						...
						'columns'			 => array(
							...
							array('name'	 => 'ldr_from_ledger_id', 'filter' => FALSE, 'value'	 => function ($data) {
									echo AccountLedger::getById($data['ldr_from_ledger_id']);
							...
							array('name'	 => 'ldr_groupby_period', 'filter' => FALSE, 'value'	 => function ($data) {
									echo $data['ldr_groupby_period'];
								}, 'sortable'			 => false, 'headerHtmlOptions'	 => array('class' => ''), 'header'			 => 'Group By'),
							array('name'	 => 'ldr_groupby_type', 'filter' => FALSE, 'value'	 => function ($data) {
									echo $data['ldr_groupby_type'];
								}, 'sortable'			 => false, 'headerHtmlOptions'	 => array('class' => ''), 'header'			 => 'Group Type'),
							...
							array('name'	 => 'adm_user', 'filter' => FALSE, 'value'	 => function ($data) {
									echo $data['adm_user'];
							...
							array('name'	 => 'ldr_created_date', 'filter' => FALSE, 'value'	 => function ($data) {
									echo date("d/M/Y", strtotime($data[ldr_created_date])) . "<br>" . date("h:i A", strtotime($data[ldr_created_date]));
							...
							array('name'	 => 'ldr_data_filepath', 'filter' => FALSE, 'value'	 => function ($data) {
									...
									{
										echo "<br><small class='text-warning'><a href='" . Yii::app()->getBaseUrl(true) . "/" . $data['ldr_data_filepath'] . "' download><i class='fas fa-file-csv font-24 color-green mb30 pl10'></i></a></small>";
									}
									else
									{
										echo "-";
...
<script type="text/javascript">
	...
	$('#LedgerDataRequests_from_ledger_id').change(function () {
		...
		if (ledgerId == 14) {
			$("#groupByVender").show("slow");
			$("#groupByPartner").hide("slow");
		} else if (ledgerId == 15) {
			$("#groupByVender").hide("slow");
	...
	$('#LedgerDataRequests_to_ledger_id').change(function () {
		var toLedgerId = $('#LedgerDataRequests_to_ledger_id').val();
		if (toLedgerId.match(14)) {
			$("#groupByVender").show("slow");
			$("#groupByPartner").hide("slow");
		} else if (toLedgerId.match(15)) {
			$("#groupByVender").hide("slow");
			$("#groupByPartner").show("slow");
		} else {
			$("#groupByPartner").hide("slow");
		...
		if (toLedgerId != '') {
			$('#LedgerDataRequests_groupby_period').val("");
			$("#groupByperiod").hide("slow");
		} else {
			$("#groupByperiod").show("slow");
		}
	});

	$('#LedgerDataRequests_group_by_type_0').change(function () {
		if ($(this).is(':checked')) {
			$('#LedgerDataRequests_groupby_period').val("");
			$("#groupByperiod").hide("slow");
		} else {
	...
	$('#LedgerDataRequests_group_by_partner_0').change(function () {
		if ($(this).is(':checked')) {
			$('#LedgerDataRequests_groupby_period').val("");
			$("#groupByperiod").hide("slow");
		} else {
