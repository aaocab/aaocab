    ...
    ' . /*
				  ...
				  //                        } */'
                        ...
                        if(data1.unbkid>0){                      
                            ...
                            location.href="' . CHtml::normalizeUrl(Yii::app()->createUrl('admin/lead/report')) . '";
                            }
                        }
                        else{ 
                            ...
                            <div class="panel-body panel-border">
                                ...
                                <div class="row new-tab-border-b">
                                    <div class="col-xs-12 col-sm-6">
										...
										<?= $form->textFieldGroup($model, 'bkg_user_name', array('label' => '','widgetOptions' => array('htmlOptions' => array('class'=>'nameFilterMask')))) ?>
                                    </div>
                                    <div class="col-xs-12 col-sm-6">
                                        <label>Last Name</label>
										<?= $form->textFieldGroup($model, 'bkg_user_lname', array('label' => '','widgetOptions' => array('htmlOptions' => array('class'=>'nameFilterMask')))) ?>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-xs-12 col-sm-6">
...
<script>
    ...
    function getRoute()
    {
        ...
        model.startCity = $('#<?= CHtml::activeId($model, "bkg_from_city_id") ?>').text();
        model.endCity = $('#<?= CHtml::activeId($model, "bkg_to_city_id") ?>').text();
...
//        var qRouteDuration = data.data.quoteddata.routeDuration;
    ...
    function convertToBooking() {
        ...
        var email = $('#BookingTemp_bkg_user_email').val();
        if ($.trim(email) != "" && !regex.test($.trim(email)))
        {
            $("#BookingTemp_bkg_user_email_em_").text("");
            $("#BookingTemp_bkg_user_email_em_").text("Please enter valid email address");
            $("#BookingTemp_bkg_user_email_em_").show();
            return false;
        }
        var href = "<?= CHtml::normalizeUrl(Yii::app()->createUrl('admin/lead/converttobooking')) ?>";
        $.ajax({
            url: href, dataType: "json", type: "POST", "data": $('#lead-form').serialize(),
            "success": function (data1) {
                if (data1.success == true) {
                    $('.bootbox').modal('hide');
                    window.location.href = '<?= Yii::app()->createUrl("admin/booking/convert", ["lead_id" => '']); ?>' + data1.leadid;//                  
                } else {
