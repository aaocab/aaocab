								...
								$this->widget('booster.widgets.TbGridView', array(
									...
									'columns'			 => array(
										array('name'	 => 'name', 'filter' => FALSE, 'value'	 => function($data) {
												if ($data['type'] == 'Vendor')
												{
													$modelv = Vendors::model()->findByPk($data['dlg_user_ref_id']);
													echo ($modelv->vnd_owner != '') ? $modelv->vnd_owner : $modelv->vnd_name;
												}
												else
												{
													echo $data['name'];
										...
										array('name'	 => 'dlg_event_id', 'filter' => FALSE, 'value'	 => function($data) {
												echo DriversLog::model()->getEventByEventId($data['dlg_event_id']);
											}, 'sortable'			 => false, 'headerHtmlOptions'	 => array('class' => 'col-xs-2'), 'header'			 => 'Event'),
										array('name'				 => 'vlg_created', 'filter'			 => FALSE, 'value'				 => 'date("d/M/Y h:i A", strtotime($data[dlg_created]))',
