                ...
                <div class="col-xs-12 col-md-7 pl0 text-center">
                    <div class="col-xs-12">
                        <div class="row rotateImg">
                            ...
                            #print_r($docModel);
                            $flag=0;
                            //$agreementFile = $docModel['vag_digital_agreement'];
                            if ($docModel['vag_approved']==0 ||$docModel['vag_approved']==2) {
                                        $flag=1;	
                            }
                            if($docModel['vag_digital_sign']!=null)
                            {
                            //$Url=$docModel['vag_digital_sign'];
                            $Url = VendorAgreement::getPathById($docModel['vag_id'], VendorAgreement::DIGITAL_SIGN);
                           ...
                           # $ImagePath="/attachments".str_replace("\\",'/',$spiltPath[1]);
                            ...
                            if($docModel['vag_digital_agreement']!=null)
                            {
                            //$Url=$docModel['vag_digital_agreement'];
                            $Url = VendorAgreement::getPathById($docModel['vag_id'], VendorAgreement::DIGITAL_AGREEMENT);
                ...
                <div class="col-xs-12 col-md-5 ">
                    ...
                    <div class="row mb5">
                     ...
                     <div class="col-xs-7 bold">
							<?php $vndName = $model->ctt_business_name!=""?  $model->ctt_business_name :  $model->ctt_first_name." ".$model->ctt_last_name?>
					        <?php echo CHtml::link($vndName, Yii::app()->createUrl("admin/vendor/view", ["id" => $docModel->vag_vnd_id]), ["class" => "", "onclick" => "return viewDetail(this)"]);?>
					 </div>
                    </div>                
                  ...
                  <?php  if ($model->ctt_city != '') {?>
					...
					<div class="row mb5">
							...
							<?php
							 if($docModel->vag_approved==1){
								         echo "<div class='col-xs-7 text-success'>Approved</div>";
							  }
							  else if($docModel->vag_approved==3){
					 ...
					 <div class="row bg-gray pt10 mt10">
							...
							<div class="col-xs-7">								
								<?php
									 if ($docModel->vag_digital_date)	{										     
                                             $docModel->vag_digital_date = DateTimeFormat::DateTimeToDatePicker($docModel->vag_digital_date);
									    }
									?>							
								<?=	$form->datePickerGroup($docModel, 'vag_digital_date', array(
											   'label'=> '',
											   'widgetOptions'	 => array(              
											...
											'value'=>$docModel->vag_digital_date==""?date("d/m/Y"):$docModel->vag_digital_date,
