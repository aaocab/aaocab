    ...
    <div class="col-md-12">
        <div class="panel" >
            <div class="panel-body panel-no-padding p0 pt10">
                ...
                <div style="width: 100%; overflow: auto;  border: 1px #aaa solid;color: #444;">
					...
					if (!empty($dataProvider))
					{
						...
						$this->widget('booster.widgets.TbGridView', array(
							...
							'selectableRows' => 2,
							'emptyText' => $active==11?'<p align="center">Search vendor with name,email and phone</p>':"No results found",
							'filter'			 => $model,
				...
				'selectableRows' => 2,
				'value'			 => '$data["vnd_id"]',
				'headerTemplate' => '<label>{item}<span></span></label>'.$data["vnd_id"],
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		$("#vnd_id_all").click(function (){
		if (this.checked)
		{
			$('#mergeform .checker span').addClass('checked');
			$('#mergeform input[name="vnd_id[]"]').attr('checked', 'true');
		} else
		{
			$('#mergeform .checker span').removeClass('checked');
			$('#mergeform input[name="vnd_id[]"]').attr('checked', 'false');
    ...
    function mergeProcess() {
        ...
        if (numrows < 1)
        {
            bootbox.alert("Please select atleast 1 vendors");
        } 
		else {
			$strDrvKeys = $keys.join();
			window.location.href = '<?php echo Yii::app()->createUrl('admin/vendor/mergevendors'); ?>?agtid=' +<?= $vendorId ?> + '&mgrvnd_id=' + $strDrvKeys;
//            var dialog1 = bootbox.confirm("Do you want to merge the selected vendors."
//                    , function (confirmed) {
//                        if (confirmed) {
//                            window.location.href = '<?php echo Yii::app()->createUrl('admin/vendor/mergevendors'); ?>?agtid=' +<?= $vendorId ?> + '&mgrvnd_id=' + $strDrvKeys;
//                        } else {
