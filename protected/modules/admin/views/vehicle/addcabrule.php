...
<div class="row">
    ...
    <div class="col-xs-12 col-md-11 col-lg-11  new-booking-list" style="float: none; margin: auto">
		...
		$form				 = $this->beginWidget('booster.widgets.TbActiveForm', array(
			'id'					 => 'cabRules-form', 'enableClientValidation' => TRUE,
			'clientOptions'			 => array(
							...
							?>	<div class="row mb15">
								...
								<div class="col-xs-12 col-sm-4">
									...
									$this->widget('booster.widgets.TbSelect2', array(
										...
										'htmlOptions'	 => array('class' => 'p0', 'style' => 'width:100%', 'onChange' => 'getAreaName(this)', 'placeholder' => 'Select Types', 'id' => 'scr_area_type', 'required' => 'required')
									));
									?>		
                        ...
                        loadSource(query, callback);
							...
							<div class="row mb15">
								...
								if ($model->isNewRecord)
								{
									...
									<div class="col-xs-12 col-sm-4"><label>Markup Amount *</label>							
										...
										<span id="markAmountErr"></span>
									</div>
								</div>
								<div class="row mb15">
									<div class="col-xs-12 col-md-4"><label>Is Supported</label>							
										<?= $form->radioButtonListGroup($model, 'scr_is_allowed', array('label' => '', 'widgetOptions' => array('htmlOptions' => [], 'data' => $isAllow), 'inline' => true)) ?>
										<span id="isAllowtErr"></span>
									</div>
								</div>
							...
							<div class="row">
								<div class="col-xs-12 text-center pb10">
									<input type="submit" value="Create" name="yt0" class="btn btn-primary pl30 pr30">
								</div>
							</div>				
						</div>
					</div>
		...
		<script>
			$("#cabRules-form").submit(function () {
				...
				$("#markTypeErr").text("");
				$("#markAmountErr").text("");
				var areaType = $("#scr_area_type").val();
				var areaName = $("#scr_area_id").val();
				var cabName = $("#ServiceClassRule_scr_scv_id").val();
				var markupType = $("#ServiceClassRule_scr_markup_type").val();
				var markupAmount = $("#ServiceClassRule_scr_markup").val();
			
			    var error = 0;
				if (areaType == '') {
					$("#areaTypeErr").text("Area Type must be filled out");
					$("#areaTypeErr").css({"color": "red"});
					error++;
				}
				if (areaName == '') {
					$("#areaNameErr").text("Area Name must be filled out");
					$("#areaNameErr").css({"color": "red"});
					error++;
				}
				if (cabName == '') {
					$("#cabNameErr").text("Cab Name must be filled out");
					$("#cabNameErr").css({"color": "red"});
					error++;
				}
				if (markupType <= 0) {
					$("#markTypeErr").text("Markup Type must be filled out");
					$("#markTypeErr").css({"color": "red"});
					error++;
				}
				if (markupAmount == '') {
					$("#markAmountErr").text("Markup Amount must be filled out");
					$("#markAmountErr").css({"color": "red"});
					error++;
				}
				if(error>0){
					return false;
