...
$cnt			 = count($model);
$checkAccess	 = Yii::app()->user->checkAccess('breakSmartMatch');
?>
					...
					<div class="row">
						<div class="col-xs-12 text-center    pt10 heading_box">
							<div class="control-label">
								TRIP ID: <?= $tripID; ?> (THIS TRIP IS NO LONGER ACTIVE) <br />
								Booking ID: 
								<b>
									<?php
									$var = explode(',', $bookingIDs);
									if (count($var) > 1)
									{
										foreach ($var as $value)
										{
											echo " ";
											echo CHtml::link($value, Yii::app()->createUrl("admin/booking/view", ["id" => $value]));
										}
									}
									else
									{

										echo CHtml::link($bookingIDs, Yii::app()->createUrl("admin/booking/view", ["id" => $bookingIDs]));
									}
									?>
								</b> was previously related to this trip and is now unlinked from that trip.
							</div>
						</div>
					</div> 
					<div class="row">
						<div class="col-xs-12 text-center    pt10 heading_box">
							No Related Active Booking found for this Booking.
						</div>
					</div>
				</div>
			</div>
		...
		else
		{
			...
			<div class="row">
				...
				$form = $this->beginWidget('booster.widgets.TbActiveForm', array(
					...
					'action'				 => Yii::app()->createUrl('admin/booking/updatevendoramount'),
					'htmlOptions'			 => array(
				...
				else
				{
					...
					<div class="col-xs-4 col-xs-offset-3"> 
						<?= $form->textFieldGroup($cabmodel, 'bcb_vendor_amount', array('label' => 'Set Vendor Amount', 'widgetOptions' => array('htmlOptions' => ['readonly' => $readonly]))) ?>
					</div>
					<?php if ($model[0]['bkg_status'] == 2 || $model[0]['bkg_status'] == 3 || $model[0]['bkg_status'] == 5)
					{
						?>
						<div class="col-xs-2 text-left pt20 mt5">
						<?php echo CHtml::button($isNew, array('class' => 'btn btn-primary', 'onClick' => 'return submitFrom()', 'value' => $saveVendorAccount)); ?>
						</div>
						<?
					}
				}
			...
			<?php
			if ($cabmodel->bcb_trip_type == 1 && $checkAccess && $model[0]['bkg_status'] == 2)
			{
				?>
				<div><a href="../booking/breakSmartMatch?bcbid=<?= $cabmodel->bcb_id ?>&bookingId=<?= $bid ?>" class="btn btn-xs btn-danger condelete" onclick="if (confirm('Are you sure you want to break smartmatch for this booking?')) {
									return true;
								} else {
									event.stopPropagation();
									event.preventDefault();
								}
								;">Break Smart Match:</a></div>
					<?php
				}
				?>
			<div class="row mb20">
								...
								<div class="row">
									<div class="col-xs-12 text-center    pt10 heading_box">
										<div class="control-label">Booking Id: 
											<b>
		<?php echo CHtml::link($val['bkg_booking_id'], Yii::app()->createUrl("admin/booking/view", ["id" => $val['bkg_id']])); ?>
											</b> 
										</div>
									</div>
								</div>
...
<script>
	function submitFrom() {
		...
		$.ajax({
			type: "POST",
			url: "<?= Yii::app()->createUrl('admin/booking/updatevendoramount') ?>",
			dataType: "json",
