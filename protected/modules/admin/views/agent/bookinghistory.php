...
<div id="agent-content" class="panel-advancedoptions" >
    <div class="panel ">    
                ...
                <div class="col-xs-12 col-sm-5 col-md-4  ">
                    <div class="row bordered  ">
                        ...
                        <div class="col-xs-12   pt10">
                            <div class="row">
                                <div class="col-xs-8 col-lg-7"><b>Total Booking Credit Applied: </b></div>
                        ...
                        <div class="col-xs-12   pt10">
                            <div class="row">
                                <div class="col-xs-8 col-lg-7 "><b>Total Transaction amount: </b></div>
                                <div class="col-xs-4 col-lg-5"><i class="fa fa-inr"></i><?= $agtData['transaction_amount'] | 0; ?></div>
                            </div>
                        </div>
		...
		<?php echo  '<b><p style="color:blue">'.$requestResult['error'].'</p></b>'; ?>
			    ...
			    var x = confirm("Are you sure you want to submit request?");
			    if(x){
                                $.ajax({
                                "type":"POST",
                                "dataType":"html",
                                "url":"' . CHtml::normalizeUrl(Yii::app()->request->url) . '",
                                 "data":form.serialize()+ "&btnType="+btn,
                        ...
                        <div class="col-xs-12  col-sm-6 col-md-6  col-lg-4">
							<?= $form->textFieldGroup($model, 'search', array('label' => "Booking or Traveller's  information", 'htmlOptions' => array('placeholder' => 'search by booking id or other information'))) ?>
                        </div>
                        ...
                        <div class="col-xs-12 col-md-6 col-sm-6  col-lg-4">
                            <div class="form-group">
								...
								$this->widget('booster.widgets.TbSelect2', array(
									...
									'htmlOptions'	 => array('class' => 'agtSelect2', 'style' => 'width:100%', 'class' => 'form-control border-radius p0', 'placeholder' => 'Select Status')
								));
								?>
                            </div>
                        </div>
                        ...
                        <div class="col-xs-12 text-center ">  
                            ...
                            <button class="btn btn-primary mt5" type="submit" style="width: 140px;"  name="bookingSearch" value="search">Search</button>
							<button type="submit" class="btn btn-success" name="bookingInvoice" value="invoice">Send me invoices for all completed & cancelled bookings in range</button>	
							<button type="submit" class="btn btn-success" name="tripSheet" value="sheet">Generate Trip Summary sheet for bookings in range</button>	
							<button type="submit" class="btn btn-success" name="proformaInvoice" value="proforma">Send Proforma invoices for pending bookings in range</button>	
						</div>
        ...
        <div class="col-md-12 ">
			...
			if (!empty($dataProvider))
			{
			    ...
			    'htmlOptions'	 => array('style' => 'width: 20px'),
						...
						['name'	 => 'bkg_user_name', 'value'	 =>	function($data) {
								if ($data['bkg_contact_id'] == null)
								{
									echo $data['bkg_user_name'];
								}
								else
								{
									$contactDetails				 = Contact::model()->getContactDetails($data['bkg_contact_id']);
									echo $contactDetails['ctt_first_name'] . " " . $contactDetails['ctt_last_name'];
									$GLOBALS['bkg_contact_no']	 = '+'.$contactDetails['phn_phone_country_code'] . $contactDetails['phn_phone_no'];
									$GLOBALS['bkg_user_email']	 = $contactDetails['eml_email_address'];
								}
							}, 'sortable'			 => true, 'headerHtmlOptions'	 => array('class' => 'col-xs-2', 'class' => 'text-center'), 'htmlOptions'		 => array('class' => 'text-center'), 'header'			 => 'Name'],
						['name'	 => 'bkg_user_email', 'value'	 => function($data) {
								if ($data['bkg_contact_id'] == null)
								{
								...
								{
									echo $GLOBALS['bkg_user_email'];
									unset($GLOBALS['bkg_user_email']);
								}
							}, 'sortable'			 => false, 'headerHtmlOptions'	 => array('class' => 'col-xs-2', 'class' => 'text-center'), 'htmlOptions'		 => array('class' => 'text-center'), 'header'			 => 'Email'],
						['name'	 => 'bkg_contact_no', 'value'	 => function($data) {
								if ($data['bkg_contact_id'] == null)
								{
								...
								{
									echo $GLOBALS['bkg_contact_no'];
									unset($GLOBALS['bkg_contact_no']);
								}
							}, 'sortable'			 => false, 'headerHtmlOptions'	 => array('class' => 'col-xs-2', 'class' => 'text-center'), 'htmlOptions'		 => array('class' => 'text-center'), 'header'			 => 'Phone'],
...
//								{
	...
	<div class="col-xs-12">
	    <button type="button" class="btn btn-success" onclick="setInvoice();">Download invoices for selected bookings</button>
...
//        $('#Booking_agt_trans_created2').val('');
    ...
    function setInvoice() {
        ...
        checked = [];
        $('input[name="bookingId[]"]').each(function (i) {
        ...
        if (checked.length == 0) {
            alert("Please select a booking for invoice.");
            return false;
        }
        if (checked.length > 0) {
            var j = 0;
            var checked1 = [];
            while (j < 100 && checkCounter < checked.length) {
                checked1.push(checked[checkCounter]);
                j++;
                checkCounter++;
            }
            var $checked2 = checked1;

            var $href = '<?= Yii::app()->createUrl("admin/agent/generateInvoice"); ?>';
            //var $href = $baseUrl + "/admin/agent/generateInvoice";
            checkCounter = 0;
            //alert($href);
            //alert($checked2);
            //window.open($href + "?bkIds=" + $checked2 , '_blank');
            window.location.href = $href + "?bkIds=" + $checked2;
        }
