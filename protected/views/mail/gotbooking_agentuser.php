...
$model->bkgInvoice->populateAmount(false,false,true,false,$model->bkg_agent_id);
...
<table style="width:98%;border: #d4d4d4 0px solid;border-collapse: collapse; font-family: 'Arial'; font-size: 12px; padding: 5px; color: #000;"  bgcolor="#fff" align="center" cellpadding="5" cellspacing="0">
    <tr>
        <td align="center"><p><strong style="font-size:16px;"><?php if($model->bkgInvoice->bkg_advance_amount>0){
                ...
                <tr>
                    <td>
                        <table width="100%" border="1" style="border-collapse: collapse;" cellpadding="5" bordercolor="#CCCCCC">
							...
							<?php if($model->bkg_agent_id > 0){?>
								<tr>
									<td><strong style="color:#7a7a7a">Booked By:</strong></td>
									<td><?= ucfirst($model->bkgAgent->agt_company) ?>( <?= ucfirst($model->bkgAgent->agt_fname) . ' ' . ucfirst($model->bkgAgent->agt_lname); ?> )</td>
								</tr>
							<?php } ?>
                        </table></td>
                    <td width="40%" align="center" valign="top">
                        <table width="100%" border="0">
                            <tr>
                                ...
                                <td><strong style="color:#7a7a7a">BOOKING  STATUS : </strong><?php if ($model->bkg_reconfirm_flag == 0) {
        ...
        <td>
                    ...
                    <td align="left">  
                       ...
                       $splRequest=0;
                        if($model->bkgUserInfo->bkg_country_code != '91' && $model->bkgPref->bkg_send_sms == 1){
                            ...
                            if($model->bkgAddInfo->bkg_spl_req_carrier != 0 || $model->bkgAddInfo->bkg_spl_req_lunch_break_time != 0){echo ';<br>';}
                            $splRequest=1;
                        }
			if ($model->bkgAddInfo->bkg_spl_req_lunch_break_time != 0)
			{
			    echo $model->bkgAddInfo->bkg_spl_req_lunch_break_time . ' minutes break during journey (Rs. '. $model->bkgAddInfo->bkg_spl_req_lunch_break_time * 5 .'/-)';
                            if($model->bkgAddInfo->bkg_spl_req_carrier != 0){echo ';<br>';}
                            $splRequest=1;
			}
