...
$stax = 'Excluded';
if ($model->bkg_is_state_tax_included == 1) {
    $stax = 'Included';
}
$ttax = 'Excluded';
if ($model->bkg_is_toll_tax_included == 1) {
    $ttax = 'Included';
}
...
$creditsused = ($model->bkg_credits_used > 0) ? $model->bkg_credits_used : 0;
...
<? } ?>
<?
$oneWayInfo = '<ol type="1" style="font-size:10px;line-height:15px;padding-left:0px;">
...
$airportInfo = '<ol type="1" style="font-size:10px; line-height:15px;padding-left:0px;">
    <li>Your reservation is subject to Gozocabs terms and conditions. (http://www.gozocabs.com/terms)</li>
...
$returnInfoPerKm = '<ol type="1" style="font-size:10px; line-height:15px;padding-left:0px;">
...
<table style="width:98%;border-collapse: collapse" align="center">
    <tr>
        <td align="center" style="padding-bottom: 8px;" colspan="4"><span style="font-size: 24px;">Thank You For Using <b style="color: #0766bb;">Gozocabs</b></span><br>
            <?
            $strBooking = "";
            if (in_array($model->bkg_status, [2, 3, 5, 6, 7]) && $advance > 0) {
                // $strBooking = "Your car booking has been confirmed successfully.";
            } else {
                if ($advance == 0 && $due > 0) {
                    $strCashback = " to save up to 50%";
                    $strBooking = "Pay at least Rs. " . $model->calculateMinPayment() . " online before " . $model->getExpTimeCashBack() . $strCashback . ". <br>Please find Payment link below in this mail.";
                }
            }
            echo $strBooking;
            ?>
    ...
    <tr>
        <td align="center" colspan="4" style="padding: 8px 0; border: #d4d4d4 1px solid; background: #f3f3f3;">
            <span style="font-size: 20px;"><b style="text-transform: uppercase;">Booking id:</b>
                <b style="color: #ff6700;"><?= Filter::formatBookingId($model->bkg_booking_id) ?></b><br><?
                if ($model->bkg_agent_id > 0) {
                    $agtArr = Agents::model()->getDetailsbyId($model->bkg_agent_id);
                    if ($agtArr['agt_type'] == 1) {
                        echo "(CORPORATE)";
                    } else {
                        echo "(AGENT)";
                    }
    ...
    <tr>
        <td align="left" colspan="4" style="padding: 10px 5px 5px;">
            <span style="color: #0766bb;"><b>TRAVELLER DETAILS</b></span></td>
    </tr>
    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px; width: 20%"><b>Name:</b></td>
        <td style="border: #d4d4d4 1px solid; padding: 5px; text-transform: uppercase;width: 30%"><?= $model->getUsername() ?></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px; width: 20%"><b>Status:</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px; text-transform: uppercase;width: 30%"><?= $model->getBookingStatus($model->bkg_status) ?></td>
    </tr>
    <tr>
        <td style="padding: 5px; border: #d4d4d4 1px solid;"><b>Pickup Date Time: </b></td>
        <td style="padding: 5px; border: #d4d4d4 1px solid;"><?= date('jS M Y (D) h:i A', strtotime($model->bkg_pickup_date)) ?></td>
        <td style="padding: 5px; border: #d4d4d4 1px solid;"><b>Pickup Address: </b></td>
        <td style="padding: 5px; border: #d4d4d4 1px solid;"><?= $model->bkg_pickup_address ?></td>
    </tr>
    <tr>
        <td style="padding: 5px; border: #d4d4d4 1px solid;"><b>Email:</b></td>
        <td style="padding: 5px; border: #d4d4d4 1px solid;"><?= $model->bkg_user_email ?></td>
        <?
        if ($model->bkg_contact_no != '') {
            ?>
            <td style="padding: 5px; border: #d4d4d4 1px solid;"><b>Phone:</b></td>
            <td style="padding: 5px; border: #d4d4d4 1px solid;"><?=
            ($model->bkg_contact_no != '') ? '+' . $model->bkg_country_code . ' ' . $model->bkg_contact_no : ''
            ?></td>
            <?
        }
        ?>
...
<table style="width:98%;border-collapse: collapse" align="center">
    <thead>
        <tr>
            <td align="left" colspan="2" style="padding: 7px 5px 5px; ">
                <span style="color: #0766bb;"><b>TRIP SUMMARY</b></span></td>
        </tr>

        <tr class="tr_gray">
            <th style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; ">From</th>
            <th style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; ">To</th>
            <th style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; ">Departure Date</th>
            <th style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; ">Time</th>
            <th style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; ">Distance</th>
            <th style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; ">Duration</th>
        </tr>
    </thead>
    <tbody>
        <?
        ...
        foreach ($model->bookingRoutes as $k => $brt) {
            ...
            <tr>
                <td style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; "><?= $brt->brtFromCity->cty_name ?>
                    <br><?= $brt->brt_from_location ?>
                </td>
                <td style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; "><?= $brt->brtToCity->cty_name ?>
                    <br><?= $brt->brt_to_location ?>
                </td>
                <td style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; "><?= DateTimeFormat::DateTimeToDatePicker($brt->brt_pickup_datetime); ?>
                </td>
                <td style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; "><?= DateTimeFormat::DateTimeToTimePicker($brt->brt_pickup_datetime); ?></td>
                <td style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; "><?= $brt->brt_trip_distance ?> Km</td>

                <td style="border: #d4d4d4 1px solid;padding: 7px 5px 5px; ">
    <?= round($brt->brt_trip_duration / 60) . ' hours';
    ?>
                </td>
...
<table style="width:98%;border-collapse: collapse;"align="center">
    <tr>
        <td align="left" colspan="2" style="padding: 7px 5px 5px; ">
            <span style="color: #0766bb;"><b>DETAILS</b></span></td>
    </tr>

    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px; width: 40%"><b>Car Package For</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px; text-transform: uppercase;width: 60%">
<?= $ct ?></td>
    </tr>

    <tr>
        <td width="40%" style="border: #d4d4d4 1px solid; padding: 5px;"><b>Cab Type </b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><?= '('. $model->bkgSvcClassVhcCat->scc_ServiceClass->scc_label .') '.$model->bkgSvcClassVhcCat->scc_VehicleCategory->vct_label . ' ' . $model->bkgSvcClassVhcCat->scc_VehicleCategory->vct_desc . ' (' . $model->bkgSvcClassVhcCat->scc_VehicleCategory->vct_capacity . ' seater)' ?></td>
    </tr>

    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Dropoff Address</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"> <?= $model->bkg_drop_address ?></td>
    </tr>
    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Journey Type</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><?= $model->getBookingType($model->bkg_booking_type) ?></td>
    </tr>
    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Booking Date</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><?= date('jS M Y (D)', strtotime($model->bkg_create_date)) ?></td>
    </tr>
    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Booking Time</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><?= date('h:i A', strtotime($model->bkg_create_date)) ?></td>
    </tr>
    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Pickup Date Time</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><?= date('jS M Y (D) h:i A', strtotime($model->bkg_pickup_date)) ?></td>
    </tr>
<? if ($model->bkg_booking_type == 2) { ?>
        <tr>
            <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Return Date Time</b></td>
            <td style="border: #d4d4d4 1px solid;padding: 5px;"><?= date('jS M Y (D) h:i A', strtotime($model->bkg_return_date)) ?></td>
        </tr>
<? } ?>
    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>KM Limit</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><?= $model->bkg_trip_distance ?> km</td>
    </tr>
    <?php
    if ($model->bkg_trip_distance > 0) {
        ?>
        <tr>
            <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Charges after <?= $model->bkg_trip_distance ?> Km</b></td>
            <td style="border: #d4d4d4 1px solid;padding: 5px;"><?= $model->bkg_rate_per_km_extra ?> / km</td>
        </tr>
        <?
    }
    if (trim($splRequest) != '') {
        ?>
        <tr>
            <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>Special Requests</b></td>
            <td style="border: #d4d4d4 1px solid;padding: 5px;"><?= $splRequest; ?></td>
        </tr>
<? } ?>
    <tr>
        <td align="left"colspan="2"style="padding: 10px 5px 5px;">
            <span style="color: #0766bb;"><b>FARE DETAILS:</b></span> </td>
    </tr>

    <tr>
        <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>Base Fare</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= number_format($model->bkg_base_amount) ?></i></td>
    </tr>
    <?php
    if ($model->bkg_additional_charge > 0) {
        ...
        <tr>
            ...
            <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= number_format($model->bkg_additional_charge) ?></i></td>
        </tr>
        <?php
    }
    if ($model->bkg_driver_allowance_amount > 0) {
        ?>
        <tr>
            <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>Driver Allowance</b></td>
            <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= number_format($model->bkg_driver_allowance_amount) ?></i></td>
        </tr>
        <?php
    }
    if ($model->bkg_discount_amount != 0) {
        ?>
        <tr>
            <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>Discount Amount (Code : <?= $model->bkg_promo1_code ?>)</b></td>
            <td style="border: #d4d4d4 1px solid;padding: 5px;">(-)Rs.<i style="font-weight: bold;"><?= number_format($model->bkg_discount_amount) ?></i></td>
        </tr>
        <?php
    }
    ?>    
    <tr>
        <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>Amount (Excl Tax)</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= ($grossAmount - $model->bkg_convenience_charge); ?></i></td>
    </tr>
    
    <tr>
        <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>Toll Tax <?= ($model->bkg_is_toll_tax_included == 1) ? "(Included)" : "(Excluded)" ?></b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= ($model->bkg_toll_tax != '') ? $model->bkg_toll_tax : 0; ?></i></td>
    </tr>
    <tr>
        <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>State Tax <?= ($model->bkg_is_state_tax_included == 1) ? "(Included)" : "(Excluded)" ?></b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= ($model->bkg_state_tax != '') ? $model->bkg_state_tax : 0; ?></i></td>
    </tr>
<?
    ...
    <? if($model->bkg_cgst > 0){ ?>
    <tr>
        <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>CGST (@<?= Yii::app()->params['cgst'] ?>%)</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= ((Yii::app()->params['cgst']/$staxrate) * $model->bkg_service_tax); ?></i></td>
    </tr>
    <? } ?>
    <? if($model->bkg_sgst > 0){ ?>
    <tr>
        <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>SGST (@<?= Yii::app()->params['sgst'] ?>%)</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= ((Yii::app()->params['sgst']/$staxrate) * $model->bkg_service_tax); ?></i></td>
    </tr>
    <? } ?>
    <? if($model->bkg_igst > 0){ ?>
    <tr>
        <td style="border: #d4d4d4 1px solid; padding: 5px;"><b>IGST (@<?= Yii::app()->params['igst'] ?>%)</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= ((Yii::app()->params['igst']/$staxrate) * $model->bkg_service_tax); ?></i></td>
    </tr>
    <? } ?>
    <? if($staxrate != 5){ ?>
    <tr>
        <td style="border: #d4d4d4 1px solid; padding: 5px;"><b><?= $taxLabel ?></b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<i style="font-weight: bold;"><?= $model->bkg_service_tax; ?></i></td>
    </tr>
    <? } ?>
    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Total cost: </b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<span style="font-weight: bold;"><?= number_format($model->bkg_total_amount) ?></span></td>
    </tr>
    <?
    if ($model1->bkg_convenience_charge > 0) {
        ?>
        <tr>
            <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Collect on delivery(COD) fee</b></td>
            <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<span style="font-weight: bold;"><?= number_format($model1->bkg_total_amount - $model->bkg_total_amount) ?></span></td>
        </tr>
        <tr>
            <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Total cost: </b><br>(if payment
                made later)</td>
            <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<span style="font-weight: bold;"><?= number_format($model1->bkg_total_amount) ?></span></td>
        </tr>
        <?
    }
    if ($creditsused > 0) {
        ?>
        <tr>
            <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Gozo Credits Used</b></td>
            <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<span style="font-weight: bold;"><?= number_format($creditsused) ?></span></td>
        </tr>
        <?php
    }
    if ($advance > 0) {
        ?>
        <tr>
            <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Advance Received</b></td>
            <td style="border: #d4d4d4 1px solid;padding: 5px;">Rs.<span style="font-weight: bold;"><?= number_format($advance) ?></span></td>
        </tr>
<? }
?> 
    <tr>
        <td style="border: #d4d4d4 1px solid;padding: 5px;"><b>Payment Due</b></td>
        <td style="border: #d4d4d4 1px solid;padding: 5px; font-size: 24px; color: #ff6700;">Rs.<span style="font-weight: bold;"><?= number_format($due) ?></span></td>
    </tr>
    <tr>
        <td colspan="2" style="padding: 5px;">
            <div style="margin-top: 5px; ">
                *Note: &nbsp; &nbsp; &nbsp;  State Tax : <?= $stax ?> &nbsp; &nbsp; &nbsp; Toll Tax : <?= $ttax ?>
            </div>
        </td>
    </tr>
    <tr>
        <td colspan="2" style="padding: 5px;">
            <div style="margin-top: 5px; ">
                <?php if ($due > 0) { ?>
                    Pay now by clicking here: <a href="<?= $payurl ?>"> <?= $payurl ?></a>
<? } ?>
            </div>
        </td>
    </tr>
    ...
    if ($rModel->rut_special_remarks != '') {
        ...
        <tr>
            <td colspan="2" style="padding: 5px;">
                <div style="margin-top: 5px; ">
    <?= nl2br($rModel->rut_special_remarks) ?>
                </div>
            </td>
        </tr>
<? } ?>
<!--    <tr style="border: #d4d4d4 1px solid; padding: 10px;">
        <td colspan="2" style="padding: 5px;">
            <div>
                <div style="float: left; margin-right:10px">
                    <img src="https://www.travelspice.com/Images/TRSLOGO.png" alt="Travel Spice" style="width:70px">
                </div>
                <div style="margin-bottom:10px; font-size:11.5px; font-family: Arial">
<?// "Book your hotel with TRAVELSPICE<sup>&reg;</sup> at the lowest price guaranteed (Find the hotel at a lower price anywhere else and claim double the difference). Use code GOZO500, to get additional Rs.500/- FREE at the hotel restaurant." ?>
                    <p></p>
                    <a href="<? // Yii::app()->createAbsoluteUrl('redirect', ['reftype' => 1, 'affid' => 1, 'bkgid' => $model->bkg_id, 'url' => 'https://www.travelspice.com/default.aspx?utm_source=affiliate&utm_medium=email&utm_campaign=gozoocmail']); ?>" style="background-color: #0766bb; padding: 10px;color: #f3f3f3;font-size: 11px;" target="_blank">BOOK NOW</a>
                </div>
            </div>
        </td>
    </tr>-->
        ...
        <td colspan="2">
            <div style="margin-top: 10px; line-height: 20px;">
                ...
                if ($model->bkg_booking_type == 1) {
                    ...
                    $dropArea = "";
                    if ($model->bkg_rate_per_km_extra != null && $model->bkg_rate_per_km_extra != "") {
                        $info = $returnInfoPerKm;
                    } else {
                        $info = "";
                    }
            ...
            <div style="margin: 20px 0 0 25px; line-height: 20px;">
                <span style="color: #909090;">Regards,</span><br>
                <b style="font-size: 16px;">-Team Gozo</b> 
            </div>
        </td>
    </tr>
</table>