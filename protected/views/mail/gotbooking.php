...
<ol type="1" style="font-size:10px; line-height:15px;padding-left:0px;">
    ...
    <ol type="a">
        ...
        <li>Drivable (' . $amount . ')  kms in your quote are estimates. You will be billed for the actual distance travelled by you. If you have not provided the exact address before your pickup or if the exact address could not be determined at time of providing you the booking amount estimate then your quote includes travel from city center to city center. Distance to be driven as quoted in the booking is only applicable for travel between the addresses, waypoints and locations as exactly specified in your itinerary.</li>
...
if ($arr['advance'] > 0)
{
    ...
    $payable = $arr['amount'] > 0 ? "<br/><b>Amount Due: </b>Rs. " . $a : "";
}
if ($arr['trip_type'] == 1)
{
...
else if($arr['booking_type'] == 9 || $arr['booking_type'] == 10 ||$arr['booking_type'] == 11){
    $bookingType  = Booking::model()->getBookingType($arr['booking_type']);
    $dropArea	  = '<br/><b>Drop Area: </b>' . $arr['dropArea'];
    $info = $returnInfoPerKm;
}
else if($arr['booking_type'] == 4){
    $info = $airportInfo; 
}
else
{
...
$msg = 'Hi ' . $arr['userName'] .
	',<br/><br/>Thank you for choosing Gozocabs! We have received your reservation request and your booking is confirmed. You can contact us anytime to inquire your booking status. The details of your reservation request are as follows:<br/>' .
