    ...
    <div class="col-xs-12">
		...
		if ($src != 1 && $platform != 3)
		{
			...
			<div class="" id="bookingDetPayNow">
				<h4 class=" text-center"><?= ucwords($model->getBookingType($model->bkg_booking_type, 'Trip')) ?> - <?= $ct ?> <nobr>(Booking ID:<?= Filter::formatBookingId($model->bkg_booking_id) ?>) </nobr>(<?= Booking::model()->getActiveBookingStatus($model->bkg_status) ?>)</h4>
				<?
				if ($paymentdone)
				{
					if ($succ == 'success')
