...
<section id="section2">
    ...
    <amp-selector class="tabs-with-flex" role="tablist">
		<div id="tab1"
			 ...
			 option
			 selected class="bgactive">Outstation Cab</div>
		<div id="tab2"
			 role="tab"
			 aria-controls="tabpanel2"
			 option class="bgnormal"><a href="/bknw/airport/<?= strtolower($cmodel->cty_alias_path) ?>">Airport Transfer</a></div>
	</amp-selector>
	<amp-selector class="tabs-with-flex" role="tablist">
		<div id="tab1"
			 role="tab"
			 aria-controls="tabpanel1"
			 option
			 selected>One Way</div>

		<div id="tab2"
			 role="tab"
			 aria-controls="tabpanel2"
			 option><a href="/bknw/multitrip/<?= strtolower($cmodel->cty_alias_path) ?>">Round Trip/Multi Way</a></div>						
	</amp-selector>
	<div class="wrraper mt10">
					...
					<section <?= $class ?>>
						...
						<div class="accordion-style2">
							<div class="table-view flex">
								<div class="table-view-left">Total kms</div> 
								<div class="table-view-right"><span class="btn-border-green"><?= $top['rut_distance']; ?></span></div>
							</div>
							<div class="table-view flex">
								<div class="table-view-left">Shared Taxi</div> 
								<div class="table-view-right"><?= ($top['flexi_price'] > 0) ? '<amp-img src="/images/rupees-amp3.png" alt="" width="9" height="12"></amp-img>' . $top['flexi_price'] : '<a href="tel:9051877000"><amp-img src="/images/call.png" alt="" width="20" height="20"></amp-img></a>'; ?></div>
							</div>
							<div class="table-view flex">
		...
		<p>  aaocab provides outstation cab booking services with driver in <?= $cmodel->cty_name; ?> for day-based rentals, one way, round trips, multicity travel and many more that are billed by custom itinerary or by day.</p>
		...
		<p>Our customers book outstation taxi services in <?= $cmodel->cty_name; ?> for trips to most nearby cities. 
			AC cabs are the most comfortable way to travel to or from <?= $cmodel->cty_name; ?>. Since the driver is a local, they are familiar with the routes and this makes it much more convenient than hiring a self-drive car. Most business travelers rent cabs for flexible travel between <?= $cmodel->cty_name; ?> and nearby towns. aaocab provides compact, sedan and SUV outstation car rentals in addition to minibus for groups (tempo traveller) and luxury cars for executives. Shared outstation taxis are also available between <?= $cmodel->cty_name; ?> and<?= $topCitiesByKm; ?>. aaocab’s cab rental services are available in over 1000 cities and on 50,000+ routes across the country. aaocab’s Outstation taxi charges are among the most reasonable, as aaocab cabs specializes in inter-city travel which gives us the greatest reach, largest supply and strong relationships with taxi operators who can deliver to you the best services with courteous and well-informed drivers. 
		...
		aaocab FLEXXI SHARE is available on many routes to and from <?= $cmodel->cty_name; ?>. Instead of booking a bus ticket to or from <?= $cmodel->cty_name; ?> you should take a look at aaocab FLEXXI SHARE.
		...
		<p>With aaocab you always get transparent billing, fair prices, 24x7 support and nationwide reach. If you have any special requirements you can always contact our customer helpdesk and we will be happy to support you.</p>


	</div>
	<div class="wrraper mt10">
		<h2>Outstation cabs in <?= $cmodel->cty_name; ?></h2>
		<p>With aaocab you can book an outstation cabs 24x7 from <?= $cmodel->cty_name; ?> to <?= $topCitiesByKm; ?>. </p>
