...
<div class="<?= $classRatesCol ?> bg-navy-<?= $sccId ?> eco-widget car-widget-4 text-center colClassRate<?= $class ?>">
	...
	<span class="fare-2">
		Estimated : <br><b class="font-18 color-orange" id="discount<?= $parentId ?>">&#x20B9;<?= $quote->routeRates->minBaseAmount . ' - &#x20B9;' . $quote->routeRates->maxBaseAmount ?><br>
			<?//= $quote->routeRates->vendorAmount . ' - &#x20B9;' . $quote->routeRates->totalAmount ?></b>
	...
	<div class="row">
		<?php
		if (in_array($model->bkg_booking_type, [1]))
		{
			?>
	...
	<span class="m0 text-uppercase text-muted <?= $showModelTier ?>" >
		...
		<input  placeholder="Select Model" class="serviceClassModel" id="service_class_area<?php echo $parentId; ?>"  name="BookingTemp[bkg_vht_id]" type="hidden" value="<?php #echo $value['id'];                                   ?>">
	</span>
