<div class="card-body timeline-panel">
	...
	<ul class="timeline ps ps--active-y timeline-widget mb0">
		...
		foreach ($model->bookingRoutes as $k => $brt)
		{
			...
			$adrsInputBox = '';
			if($brt->brt_to_city_id==$model->bookingRoutes[$k+1]->brt_to_city_id && $model->bookingRoutes[$k-1]->brt_to_city_id==null && $model->bookingRoutes[$k+1]->brt_to_city_id!=null)
			{
					$adrsInputBox = 'hide';		
			}
			...
			if($k>0)
			{
				$locAddress	 = ($model->bookingRoutes[0]->brt_from_location == '') ? $model->bookingRoutes[0]->brtFromCity->cty_name : $model->bookingRoutes[0]->brt_from_location;
				$locAddress	 .= ($model->bookingRoutes[0]->brt_from_latitude > 0) ? '' : ', ' . $model->bookingRoutes[0]->brtFromCity->cty_name;
			}
			?>

			<li class="timeline-item timeline-icon-success active <?=$adrsInputBox?>">
				<?
				
				?>
				<div class="p5 timeline-left">
					<?= DateTimeFormat::DateTimeToDatePicker($model->bkg_pickup_date); ?><br>
					<?= DateTimeFormat::DateTimeToTimePicker($model->bkg_pickup_date); ?>
					<br><br></div>

				<div class="timeline-hori"><img src="/images/bx-tachometer.svg" alt="img" width="11" height="11" class="bx-rotate-90"> <?=$model->bkg_trip_distance?> km</div>
<!--				<div class="timeline-hori-2"><img src="/images/bx-info-circle.svg" alt="img" width="11" height="11" class="bx-rotate-90"> <?//= BookingRoute::model()->formatTripduration($model->bkg_trip_duration, 1); ?></div>-->
			...
			if ($k == ($cntBrt - 1))
			{
				...
				<li class="timeline-item timeline-icon-primary active">
					...
					?>
					<div class="p5 timeline-left">	
					<?= DateTimeFormat::DateTimeToDatePicker($expArrivedate); ?><br>
					<?= DateTimeFormat::DateTimeToTimePicker($expArrivedate); ?>
					</div>
					<br>	
