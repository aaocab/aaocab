...
$orderAmount			 = ($bkgMinAmount == '') ? $model->bkgInvoice->calculateMinPayment() : $bkgMinAmount;
//$maxPaymentWithDiscount	 = round($model->bkgInvoice->bkg_total_amount) - $advance;
...
<div id="menu-list-bottom2" data-selected="menu-components" data-height="480" class="menu-box menu-bottom" style="display:none; transition: all 300ms ease 0s;">
	...
	<script type="text/javascript">
		...
		var netAmount = <?php echo $maxPaymentWithDiscount ?>;
		...
		var readOnlyPayTrainman = '<?php echo $readOnlyPayTrainman ?>';
		var defaultPaymentGateway = '<?php echo $defaultPaymentGateway; ?>';
		...
		{
			var defPayuBolt = '<?php echo Yii::app()->params['enablePayuBolt'] ?>';
...
//            opt6Click();
	...
	<script type="text/javascript">
		...
		{ //debugger;
			$.ajax({
				"type": "POST",
				"dataType": "json",
				"url": "<?= CHtml::normalizeUrl(Yii::app()->createUrl('booking/paymentv3')); ?>",
				"data": $("#payment-form1").serialize(),
				"beforeSend": function ()
				{
					ajaxindicatorstart("");
	...
	<script type="text/javascript">
		...
		function processEasebuzz()
		{
			$('#BookingInvoice_paymentType').val('23');
			useCredit();
			calculatePayable();
			$('#BookingTrail_bkg_tnc').prop('checked', true);
			$("#BookingInvoice_payubolt").val(1);
		...
		function processOther()
		{
			$.ajax({
				"type": "POST",
				"dataType": "json",
				"url": "<?php echo CHtml::normalizeUrl(Yii::app()->createUrl('booking/paymentv3')); ?>",
				"data": $("#payment-form1").serialize(),
				"beforeSend": function ()
				{
					ajaxindicatorstart("");
		...
		function launchEasebuzz() {
			...
			$.ajax({
				...
				"success": function (data2)
				{
					...
					var responseData = JSON.parse(data2);

					if (responseData.status == 1) {
						var easebuzzCheckout = new EasebuzzCheckout(responseData.key, responseData.env)
						var options = {
							access_key: responseData.accesskey, // access key received via Initiate Payment
							onResponse: (response_data) => {
								$('#bkPaymentModal').modal().hide();
								...
								$.ajax({
									...
									"dataType": "json",
									"url": "<?php echo CHtml::normalizeUrl(Yii::app()->createUrl('payment/response')); ?>",
									"data": {'response': response_data, 'ptpid': 23, 'bolt': 1},
									"success": function (data4)
									{
										location.href = data4.url;
                ...
                //    debugger;
						...
						document.getElementById('loading1').innerText = JSON.stringify(responseData);
