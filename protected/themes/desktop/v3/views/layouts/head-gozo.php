...
Yii::app()->clientScript->registerScriptFile(Filter::getBaseUrl() . '/js/gozo/v3/home.js?v=' . $version, CClientScript::POS_HEAD);
Yii::app()->clientScript->registerPackage("webV3End");
Yii::app()->clientScript->registerScriptFile(Filter::getBaseUrl() . '/js/gozo/rap.js?v=' . $version, CClientScript::POS_HEAD);
?>
...
<html lang="en">
    <head>
	   ...
	   <link rel="preconnect" href="<?= ASSETS_DOMAIN ?>">
	   ...
	   if(Filter::getOriginHost() != 'www.gozocabs.com')
	   {
		   $canonical = "https://www.gozocabs.com".Yii::app()->getRequest()->getUrl();
...
<?= (UserInfo::getGA4UserId() > 0) ? "initUserId('" . UserInfo::getGA4UserId() . "');" : "" ?>
	   ...
	   <script>
		...
		function ajaxindicatorstart(text)
		{
		 ...
		 {
			jQuery('body').append('<div id="resultLoading" style="display:none"><div><img src="<?php echo Filter::getBaseUrl() ; ?>/images/ajax-loader1.gif"><div>' + text + '</div></div><div class="bg"></div></div>');
		 }
    ...
    <script>
	 ...
	 function getCbmUrl(reftype)
	 {
		...
		{
		 case 1:
			var href2 = "<?php echo Filter::createUrl('scq/newBookingCallBack') ?>";
			break;
		 case 2:
			var href2 = "<?php echo Filter::createUrl('scq/existingBookingCallBack') ?>";
			break;
		 case 3:
			var href2 = "<?php echo Filter::createUrl('scq/vendorAttachmentCallBack') ?>";
			break;
		 case 4:
			var href2 = "<?php echo Filter::createUrl('scq/existingVendorCallBack') ?>";
			break;
		 default:
			var href2 = "<?php echo Filter::createUrl('scq/newBookingCallBack') ?>";
			break;
...
//                        var href2 = "<?php //echo Filter::createUrl('users/signin')                          ?>";
//                        $.ajax({
//                            "url": href2,
//                            "data": {"signup": 3},
//                            "type": "GET",
...
//                        });
    ...
    <script>
	 ...
	 function getVisitor()
	 {
		...
		if (!vid)
		{
		 var href = "<?php echo Filter::createUrl('users/regVisitor') ?>";
		 $.ajax({
			type: 'GET',
			url: href,
			async: true,
	 ...
	 function showLogin(callback, showPhone = 0, form = null)
	 {
		...
		trackPage('<?= Filter::createUrl("users/signin"); ?>');
		$.ajax({type: 'GET',
		 url: '<?= Filter::createUrl("users/signin"); ?>',
		 data: param,
		 dataType: "html",
		 "beforeSend": function ()
		 {
	 ...
	 function refreshNavBar()
	 {
		try
		{
		 ...
		 {
			$.ajax({type: 'GET',
			 url: '<?= Filter::createUrl("users/refreshNav"); ?>',
			 dataType: "html",
			 success: function (data)
			 {
		 ...
		 {
			$.ajax({type: 'GET',
			 url: '<?= Filter::createUrl("index/refreshHomeNav"); ?>',
			 dataType: "html",
			 success: function (data)
			 {
		 ...
		 {
			$.ajax({type: 'GET',
			 url: '<?= Filter::createUrl("booking/refreshTravellerInfo"); ?>',
			 dataType: "html",
			 success: function (data)
			 {
	 ...
	 $(".batchPopup").click(function ()
	 {
		...
		var href2 = "<?php echo Filter::createUrl('index/Offer') ?>";
		$.ajax({
		 "url": href2,
		 "type": "GET",
		 "dataType": "html",
