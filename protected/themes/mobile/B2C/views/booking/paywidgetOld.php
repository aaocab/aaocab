...
<div class="pay-panel mb0">
	<a href="#" data-menu="menu-list-bottom2" id="payBoxBtn">Pay <span>&#x20b9</span><span id="minpayval"><?= $orderAmount ?></span></a>
</div>
<div id="menu-list-bottom2" data-selected="menu-components" data-height="480" class="menu-box menu-bottom" style="display:none; transition: all 300ms ease 0s;">
		...
		if ($minDiff > 0)
		{
			if ($model->bkgInvoice->bkg_due_amount > 0)
			{
				...
				<div class="content p0 bottom-0" id="payment">
					...
					<?= $form->hiddenField($model, 'hash', ['id' => 'hash5','value'=>"$hash"]); ?>
					...
					<input type="hidden" class=" " name="max_amount" id="max_amount" value="<?= $maxPaymentWithDiscount; ?>">

					<input type="hidden" id="bkghash" name="bkghash"  value="<?= $hash ?>">
						...
						<div class="search-results  ">	
							...
							<div data-filter-item="" data-filter-name="intcredit indcredit banking lazy paytm mbk epay"  class="disabled-search-item disabled-search-item" >
								...
								<div class="content p0 bottom-0" >
									...
									<div class="showcash form-control " style="display: none"><?= $model->bkgInvoice->bkg_due_amount ?></div>
									<div id = "ppayment"  class="content p0 bottom-0 mt15">
										...
										<div class="content p0 bottom-0">
											<div class="one-half input-simple-1 has-icon input-green bottom-15">
												<?= $form->numberField($model->bkgInvoice, 'partialPayment',['class' => 'pl10 ', 'placeholder' => "Partial Payment", 'min' => $orderAmount, 'max' => $maxPaymentWithDiscount] + $readOnlyPartialPay) ?>
											</div>
								...
								<div class="content p0 bottom-0 top-10">
									...
									<label>
										...
										if (in_array($model->bkg_flexxi_type, [1, 2]))
										{
											...
											<?= 'I agree to the Gozo <a href="javascript:void(0);" class="termscls" >terms and conditions</a> & <a href="javascript:void(0);" onclick="openflexxiterms()" >Flexxi terms and conditions</a>' ?>
											<?
										}
										else
										{
	...
	<input id="minipay"  type="hidden" value="<?= $orderAmount ?>">
	<input id="dueAmountWithoutCOD" type="hidden" value="<?= $model->bkgInvoice->bkg_due_amount; ?>">

	<script type="text/javascript">
		var booknow = new BookNow();
		$netAmount = <?= $maxPaymentWithDiscount ?>;
		var bid = '<?= $model->bkg_id ?>';
		var hsh = '<?= Yii::app()->shortHash->hash($model->bkg_id) ?>';
		var dollarToRupeeRate =<?= Yii::app()->params['dollarToRupeeRate'] ?>;
		$readOnlyPayTrainman = '<?= $readOnlyPayTrainman ?>';
...
//			$('#cardAsk').show();
	...
	<script type="text/javascript">
		...
		if(specialStatus!=1)
		{  
			booknow.showErrorMsg("Please save your special requests preferences first. We need to know passenger + luggage information before you can proceed to pay.");
			
			$(".menu-hide").trigger("click");
			$("#accordion-3").css("display","block");
			$("#accordion-1").css("display","none");
			$('html, body').animate({scrollTop: $("#accordion-3").offset().top - 80},200);
