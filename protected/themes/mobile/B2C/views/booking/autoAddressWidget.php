...
<div class="content-boxed-widget p0 accordion-path newAddress">
    <div class="accordion accordion-style-0">
	<div class="accordion-border">
	    <a href="javascript:void(0)" class="font18" data-accordion="accordion-59"><span class="uppercase">Update Address</span><i class="fa fa-plus"></i></a>
...
?>
		...
		<div class="col-xs-12 col-sm-12 journey-p">
			...
			foreach ($model->bookingRoutes as $key => $brtRoute)
			{
			    ...
			    <div class ="row">
				<div class="input-simple-1 textarea has-icon bottom-10">
				    <i class="fa fa-edit"></i><em class="color-highlight">Pickup <?= $addressLabel ?> for <?= $brtRoute->brtFromCity->cty_name ?> *:</em>
				    ...
				    echo $form->textField($brtRoute, "[0]brt_from_location", ['id' => "locFont_$key", 'class' => "textarea-simple-1 $autocompleteFrom brt_location_$key", 'readonly' => $readOnly, 'required' => $required, "autocomplete" => "section-new", 'placeholder' => "Pickup Address  (Required)", "data-lockey" => $key] + $locFrom);
				    $form->textField($model, 'bkg_pickup_address');
				    ?>
				</div>                    
...
//			}
			    ...
			    <div class="row mt10">		
					<div class="input-simple-1 textarea has-icon bottom-10 mt10">
					    <i class="fa fa-edit"></i><em class="color-highlight">Drop <?= $addressLabel ?> for <?= $brtRoute->brtToCity->cty_name ?> <?= $optReq ?>:</em>
					    ...
					    echo $form->textField($brtRoute, "[$key]brt_to_location", ['id' => "locFont_$key1", 'class' => "textarea-simple-1 $autocompleteTo brt_location_$key1", "autocomplete" => "new-password", 'required' => $required, 'placeholder' => $placeHolder, "style" => "margin-bottom:0", "data-lockey" => $key1] + $locTo);
					    echo "<span class='hide' style='color:#a94442' id='skipAddErr" . $key1 . "'>Please select any location</span>";
					    if ($key1 == $cntRt)
					    {
						$form->textField($model, 'bkg_drop_address');
	...
	<!--						<div class="col-xs-2"><span class="autoMarkerLoc <?= $locMarkerTo ?>" data-lockey="<?= $key1 ?>" data-toggle="tooltip" title="Select destination location on map"><img src="/images/locator_icon4.png" alt="Precise location" width="30" height="30"></span></div>-->
		    ...
		    <? } ?>
		    <button type="button" id="saveAddreses" class="uppercase btn-orange shadow-medium" name="saveAddreses" onclick="saveAddresses();">Save Addresses</button>
		</div>
...
//                    });
				...
				$('#info-map-marker').click();
					...
					$('#saveAddreses').removeClass("btn-orange");
					$('#saveAddreses').addClass("btn-default");
