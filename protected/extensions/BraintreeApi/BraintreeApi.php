...
class BraintreeApi
{
	...
	public function getOptions($values)
	{
		...
		if (isset($values['orderId']))
			$this->options['orderId'] = $values['orderId'];
		if (isset($values['creditCard']))
			$this->getCreditCard($values['creditCard']);
		if (isset($values['customer']))
			$this->options['customer'] = $values['customer'];
		if (isset($values['billing']))
			$this->options['billing'] = $values['billing'];
		if (isset($values['shipping']))
			$this->options['shipping'] = $values['shipping'];

		//For storing payment method in vault
		if (isset($values['customerId']))
			$this->options['customerId'] = $values['customerId'];
	}
