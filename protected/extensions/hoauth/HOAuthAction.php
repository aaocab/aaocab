...
class HOAuthAction extends CAction
{
	...
	protected function oAuth($provider)
	{
		try
		{
			...
			else
			{
				...
				if (!$oAuth->isBond || !$user)
				{
					...
					if (!isset($user))
					{
						$type		 = SocialAuth::getTypeByProvider($oAuth->provider);
						$contactSet	 = Contact::createBySocialProfile($oAuth->getProfile(), $type);
						$contactId	 = $contactSet->getData()['contactId'];
						$user		 = Users::createbyContact($contactId);
		...
		if (isset($_REQUEST['isFlexxi']) && $_REQUEST['isFlexxi'] == 1)
		{
			Yii::app()->controller->redirect(['index/index', 'isFlexxi' => 1]);
		}
