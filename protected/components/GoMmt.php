...
class GoMmt extends CComponent
{
	...
	const NIGHT_ALLOWENCE_CHARGE = 1;
	const STATE_TAX_CHARGE	   = 2;
	const TOLL_TAX_CHARGE		   = 3;
	const AIRPORT_TOLL_CHARGE	   = 4;
	const AIRPORT_ENTRY_FEE	   = 5;
	const WAITING_CHARGE		   = 6;
...
//				break;
	...
	public static function processQuote($model, $cabId = 0, $processCnt = 0, $isAllowed = false)
	{
		...
		if ($cabId <= 0)
		{
			$cabId = [1, 2, 3, 5, 6, 14, 15, 16, 72, 73, 74, 75, 100];
			#$cabId = [1, 2, 3, 5, 6, 72, 73, 74, 75];
			if (in_array('CO', $model->search_tags) || in_array('B2B', $model->search_tags))
			{
				$cabId	 = [14, 15, 16, 100];
	 ...
	 * @return boolean
	 */
	public static function isCashBookingAllowed($fromCity, $tripType, $vehicleId, $toCity, $partnerId, $requestType, $obj, $model)
	{
		$type		   = PartnerAreaRule::ADVANCE_AMOUNT_CHECK;
		$advanceAmount = (($obj->total_fare) - ($obj->amount_to_be_collected));
		$result		   = false;
		/* @var $getAreaRule PartnerAreaRule */
		$getAreaRule   = PartnerAreaRule::getByCity($fromCity, $tripType, $vehicleId, $toCity, $partnerId, $type);
		if ($getAreaRule != [])
		{
			if ($getAreaRule['par_min_advance'] != null)
			{
				$arrRules = CJSON::decode($getAreaRule['par_min_advance']);
				$advPercentageValue = (($arrRules['type'] == 1) ? round($obj->total_fare * ($arrRules['value'] / 100)) : $arrRules['value']);

				if($requestType == GoMmt::CONFIRM_REQUEST)
				{
					if ($advanceAmount >= $advPercentageValue && $obj->platform_fee == 0 && $obj->booking_gst == 0)
					{
						$result = false;
					}
					else
					{
						//$amount = ROUND($obj->amount_to_be_collected - $obj->total_fare);
						$model->bkgInvoice->bkg_partner_extra_commission = $obj->platform_fee + $obj->booking_gst;
						$model->bkgInvoice->bkg_vendor_collected = $obj->amount_to_be_collected;
						$model->bkgInvoice->bkg_total_amount = $obj->amount_to_be_collected;
						$model->bkgInvoice->bkg_advance_amount = 0;
						$model->bkgInvoice->bkg_credits_used = 0;
						$model->bkgInvoice->bkg_due_amount = $obj->amount_to_be_collected;
				...
				{
					if ($arrRules['type'] == 1 && $arrRules['value'] == 0)
					{
						$result = true;
					}
				}
	...
	public static function isAllowedCity($bkgFromCity, $partnerId)
	{
		...
		if ($isFromCity == true && ($partnerId == 18190 || $partnerId == 30228))
		{
			$result = true;
		}
		return $result;
