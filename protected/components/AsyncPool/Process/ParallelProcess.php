...
namespace components\AsyncPool\Process;
...
class ParallelProcess implements \Spatie\Async\Process\Runnable
{

	public $processId, $identifier;
	protected $process;
	protected $id;
	protected $pid;
	protected $output;
	...
	use \Spatie\Async\Process\ProcessCallbacks;
	...
	public function getOutput()
	{
		if (!$this->output)
		{
			$fileName = '/runtime/' . $this->getPid() . "." . $this->identifier;
			
			if (file_exists($fileName))
			{
				$processOutput = file_get_contents($fileName);
			}

			$this->output = @unserialize($processOutput);

			if (!$this->output)
			{
				$this->errorOutput = $this->process->getOutput();
			}
